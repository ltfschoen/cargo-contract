...
macro_rules! test_tests {
    ( $($fn:ident),* ) => {
        #[test]
        fn test_tests() {
            crate::tests::with_tmp_dir(|tmp_dir| {
                let ctx = crate::tests::CmdTestContext::new(tmp_dir, "build_test")?;
...
fn testing_without_flag_must_work(manifest_path: &ManifestPath) -> Result<()> {
    ...
    // TODO - should `ExecuteArgs` be replaced with `TestCommand`
    // whereever it is used in this file?
    ...
    let results = contract_build::exec(args).expect("test failed");
